...
void LPS::begin(uint8_t &device_address){
  ...
  DW1000Ng::setPreambleDetectionTimeout(0); //15
...
bool LPS::isMyTurn(){
  ...
  if(receiveAck){
    ...
    receiveAck = false;
    LPS::commitData();
    if(rx_buff[7] == ANCHOR_MAIN && rx_buff[9] == START_TWR){
      output = true;
    }
  }
  return output;
...
bool LPS::isRangeReport(uint8_t currentAnchor){
   bool output = false;
  if(sentAck){
    sentAck = false;
    LPS::startReceive();
  ...
  if(receiveAck){
    receiveAck = false;
    LPS::commitData();
    if(rx_buff[7] == currentAnchor && rx_buff[9] == RANGE_REPORT){
      output = true;
    }
  }
  return output;
...
double LPS::getDistance(){
  double output = 0.0;
  memcpy(&output, &rx_buff[10], sizeof(double));
  return output;
}

void LPS::setTimePollSent(){
  timePollSent = DW1000Ng::getTransmitTimestamp();
}
...
void LPS::calculatePosition(double &x, double &y){
  ...
  double C = (distance[0]*distance[0]) - (distance[1]*distance[1]) - (position_self.x*position_self.x) + (position_B.x*position_B.x) - (position_self.y*position_self.y) + (position_B.y*position_B.y);
  double D = ( (-2*position_B.x) + (2*position_C.x) );
  double E = ( (-2*position_B.y) + (2*position_C.y) );
  double F = (distance[1]*distance[1]) - (distance[2]*distance[2]) - (position_B.x*position_B.x) + (position_C.x*position_C.x) - (position_B.y*position_B.y) + (position_C.y*position_C.y);
