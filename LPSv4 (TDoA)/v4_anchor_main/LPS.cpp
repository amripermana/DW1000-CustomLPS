...
void LPS::begin(uint8_t &device_address){
  ...
  DW1000Ng::setSfdDetectionTimeout(1089); //273
  DW1000Ng::setReceiveFrameWaitTimeoutPeriod(4000); //2000
...
void LPS::sendBroadcast(){
  ...
  // memcpy(&tx_packet[9], &timeSent, sizeof(uint64_t));

  DW1000Ng::setTransmitData(tx_packet, LEN_DATA);
  DW1000Ng::startTransmit(TransmitMode::DELAYED);
  // DW1000Ng::startTransmit();
  // DW1000Ng::setTransmitData(bc_packet, 10);
  // DW1000Ng::startTransmit();
  // Serial.println("CMD SEND");
}
