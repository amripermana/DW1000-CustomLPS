...
bool LPS::startRTLS(uint8_t target, double &outX, double &outY){
  ...
  if(DW1000NgRTLS::receiveFrame()){
    ...
    if(recv_data[0] == BLINK) {
        DW1000NgRTLS::transmitRangingInitiation(&recv_data[2], target_id);
...
void LPS::sendXY(uint8_t target, double x, double y){
  tx_packet[5] = target;
  tx_packet[7] = self_address;
  tx_packet[9] = XY_REPORT;
  memcpy(&tx_packet[10], &x, sizeof(double)); 
  memcpy(&tx_packet[18], &y, sizeof(double));
  DW1000Ng::setTransmitData(tx_packet, LEN_DATA);
  DW1000Ng::startTransmit();
}
